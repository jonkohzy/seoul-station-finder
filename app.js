(() => {
  ...
  svg.querySelectorAll('#name_en text, #name_en_overlap text').forEach(el=>{
    ...
    const station={id,key,name,el,terms:StationSearch.terms(name),box:{x:left,y:top,width:right-left,height:bottom-top},x:(left+right)/2,y:(top+bottom)/2,planned:el.classList.contains('st56')};
  ...
  function render(){trip?.positionPins(scale,tx,ty);stage.style.transform=`translate(${tx}px,${ty}px) scale(${scale})`;$('zoom-level').textContent=Math.round(scale*100)+'%';if(selected){$('map-pin').style.left=(selected.x*scale+tx)+'px';$('map-pin').style.top=((selected.box.y-9)*scale+ty)+'px';}}
  ...
  $('recenter').addEventListener('click',()=>{if(selected){trip?.stopAutoFit();moveTo(selected.x,selected.y,Math.min(1.65,(viewport.clientWidth-100)/(selected.box.width+70)));}});
  function zoom(factor,x=viewport.clientWidth/2,y=viewport.clientHeight/2){trip?.stopAutoFit();cancelAnimationFrame(animation);const next=Math.max(fitScale(),Math.min(3,scale*factor));tx=x-(x-tx)*next/scale;ty=y-(y-ty)*next/scale;scale=next;clamp();render();}
  ...
  viewport.addEventListener('pointerdown',e=>{if(e.button!==0||e.target.closest('.saved-map-pin'))return;trip?.stopAutoFit();cancelAnimationFrame(animation);pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});gesture=gestureState();moved=false;viewport.setPointerCapture(e.pointerId);viewport.classList.add('dragging');});
  ...
  viewport.addEventListener('keydown',e=>{const actions={'+':()=>zoom(1.4),'=':()=>zoom(1.4),'-':()=>zoom(1/1.4),'0':fullMap,ArrowLeft:()=>{tx+=80;},ArrowRight:()=>{tx-=80;},ArrowUp:()=>{ty+=80;},ArrowDown:()=>{ty-=80;}};if(actions[e.key]){e.preventDefault();trip?.stopAutoFit();cancelAnimationFrame(animation);actions[e.key]();clamp();render();}});
  let previous={w:viewport.clientWidth,h:viewport.clientHeight};new ResizeObserver(()=>{const w=viewport.clientWidth,h=viewport.clientHeight;tx+=(w-previous.w)/2;ty+=(h-previous.h)/2;previous={w,h};if(!trip?.refit()){clamp();render();}}).observe(viewport);
  ...
  trip=createTripPins({stations,svg,overlay,viewport,getSelected:()=>selected,focusStation,
    setView(x,y,s){cancelAnimationFrame(animation);scale=Math.max(.01,Math.min(1.65,s));tx=x;ty=y;render();},
    repaint:render,announce:message=>{$('announcement').textContent=message;}});
  updateResults();if(!trip.refit())moveTo(3300,2600,Math.min(viewport.clientWidth/2100,viewport.clientHeight/1650),false);
